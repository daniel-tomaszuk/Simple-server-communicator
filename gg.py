...
def log_in(args):
    ...
    if len(password) >= 8:
        ...
        for gg_user in gg_users:            
            ...
            if args.user == gg_user.email and \
                    check_password(password, gg_user.hashed_password()):
                print("Logged as {}".format(args.user))
                disconnect_db(cursor, cnx)
                # logged in  
                return gg_user.id
            # if there is an user to log into but password is incorrect
            elif args.user == gg_user.email and \
                    not(check_password(password, gg_user.hashed_password())):
...
def new_pass(args):
    ...
    if log_in_result and args.new_pass:
...
def delete(args):
    log_in_result = log_in(args)
    if log_in_result and args.delete:
        cnx = connect_db()        
        cnx.autocommit = True
        cursor = cnx.cursor()        
        user = User.load_by_id(log_in_result, cursor)
        in_del = input("Do you want to"
                       " delete user {}? y/n ".format(user.username))
        if in_del.lower() == 'y':
            user.delete(cursor)
            disconnect_db(cursor, cnx)
            return True
...
def list(args):
    log_in_result = log_in(args)
    if log_in_result and args.list:
        cnx = connect_db()        
        cnx.autocommit = True
        cursor = cnx.cursor() 
        gg_users = User.load_all_users(cursor)
...
def edit(args):
    log_in_result = log_in(args)
    if log_in_result and args.edit:
        cnx = connect_db()        
        cnx.autocommit = True
        cursor = cnx.cursor()
        user = User.load_by_id(log_in_result, cursor)
...
def send_to(args):
    log_in_result = log_in(args)
    if log_in_result and args.send_to:
        cnx = connect_db()        
        cnx.autocommit = True
        cursor = cnx.cursor()
        
        to_user = input("Write an email of the user"
                        " that you want to send message to: ")
...
def list_messages(args):
    log_in_result = log_in(args)
    if log_in_result and args.list_messages:
        cnx = connect_db()        
        cnx.autocommit = True
        cursor = cnx.cursor()    
...
def main():
    ...
    parser.add_argument('-p', '--password', action='store_true',
                        dest='password', help='hidden password prompt')
    
    # parser.add_argument("-n","--new-pass",
    # help="sets new password while logging")
    group2.add_argument('-n', '--new-pass', action='store_true',
                        dest='new_pass',
                        help="sets new password while logging")
    group2.add_argument("-l", "--list", action="store_true",
                        help="list all users")
    group2.add_argument("-lm", "--list_messages",
                        action="store_true",
                        help="list all messages for logged user")
    group2.add_argument("-d", "--delete",
                        action="store_true",
                        help="delete logged user")
    group2.add_argument("-e", "--edit",
                        action="store_true",
                        help="edit user email")
    
    # group2.add_argument("-t","--to", help="to whom send your message")
    # group2.add_argument("-s","--send",
    # action="store_true", help="send message")
    
    group2.add_argument("-st", "--send_to",
                        action="store_true",
...
#     help = parser.print_help()
    ...
    if (not(args.new_pass)) and (not(args.delete)) and (not(args.list)) and \
            (not(args.edit)) and (not(args.send_to)) and \
            (not(args.list_messages)):
        log_in(args)
    elif args.new_pass:
        new_pass(args)        
    # delete user    
